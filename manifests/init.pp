...
#
# [*puppetdb_ssl_verify*]
#   (bool, absolute path) whether PuppetDB uses SSL or not, true or false, or the path to the puppet CA
...
class puppetboard(
  ...
  if is_bool($puppetdb_ssl_verify) {
	validate_bool($puppetdb_ssl_verify)
  } else {
	validate_absolute_path($puppetdb_ssl_verify)
  }
